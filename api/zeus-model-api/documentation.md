# Zeus Model API API Documentation
...
## Base URL
```
/api/v1/zeus model api
```

## Endpoints

### GET /zeus model api
Get all zeus model api records
...
### GET /zeus model api/{id}
Get a specific zeus model api record by ID
...
### POST /zeus model api
Create a new zeus model api record
...
### PUT /zeus model api/{id}
Update a zeus model api record
...
### DELETE /zeus model api/{id}
Delete a zeus model api record
...
### Zeus Model API Schema
```typescript
interface ZeusModelAPI {
  id: string;
  name: string;
  description?: string;
  createdAt: Date;
...
```bash
npm install zeus model api-sdk
```

## SDK Usage

```typescript
import { ZeusModelAPIAPI } from 'zeus model api-sdk';

const api = new ZeusModelAPIAPI();
