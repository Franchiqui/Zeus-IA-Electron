...
`;
  ...
  return {
    'electron/main.js': mainJs,
    'electron/preload.js': preloadJs,
  };
}
...
export function getPackageJsonContent(selectedTemplate: string, appName: string, authMethod?: string, features: string[] = []): string {
  switch (selectedTemplate) {
    case 'next-js': {
      ...
      const electronScripts = `"electron:dev": "concurrently \\"next dev -p 3002\\" \\"wait-on http://localhost:3002 && cross-env ELECTRON_TARGET=desktop ELECTRON_DEV_PORT=3002 node scripts/run-electron-dev.js\\"",
          "electron:dev:standalone": "concurrently \\"next dev -p 3002\\" \\"wait-on http://localhost:3002 && cross-env ELECTRON_TARGET=desktop ELECTRON_DEV_PORT=3002 electron .\\"",
      ...
      const electronBuildConfig = `"main": "electron/main.js",
       "build": {
...
`;
  ...
  switch (selectedTemplate) {
    case 'next-js':
      templateDetails = `
        Framework: Next.js 13+ (React) with App Router packaged with Electron for DESKTOP
        Language: ${language}
        Folder Structure: YOU MUST use the Next.js 13+ App Router with the following mandatory structure, PLUS Electron-specific files:
 ...
 - The "files" key MUST contain an object where the keys are the file paths and the values are the full file content.
  ...
  if (fileListResult.usage && userId) {
    try {
      ...
      console.error('[Usage Recording] Error registrando consumo en Lista de Archivos (Escritorio):', usageError);
    }
  }
...
`;
  ...
  // Ensure Electron files exist for desktop apps
  const electronFiles = generateElectronFiles(appName);
  for (const [electronPath, electronContent] of Object.entries(electronFiles)) {
    if (!fileMap[electronPath]) {
      fileMap[electronPath] = electronContent;
      console.log(`DEBUG: Injected missing Electron file: ${electronPath}`);
    }
  }
...
Framework: Next.js 13+ with App Router packaged with Electron for desktop
Styles: Tailwind CSS
---
...
`;
        ...
        if (fileContentResult.usage && userId) {
          try {
            ...
            console.error(`[Usage Recording] Error registrando consumo en archivo ${filePath} (Escritorio):`, usageError);
          }
        }
  ...
  console.log('[generate-app-escritorio] 💾 Guardado de ZIP en PocketBase deshabilitado. Archivos disponibles en disco local.');
