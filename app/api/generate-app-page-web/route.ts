...
import { getModelsForUser, StorageModelConfig } from '@/api/utils';
...
async function createProjectStructure(basePath: string, files: Record<string, { content: string | object }>, projectId?: string) {
  ...
  for (const [relativePath, fileData] of Object.entries(files)) {
    ...
    if (fileData && fileData.content) {
      ...
      // Apply new code correction method directly for app/page.tsx
      if (relativePath === 'app/page.tsx') {
        console.log(`🔧 Applying new code correction to: ${relativePath}`);
        try {
          // Remove metadata/generateMetadata from page (metadata lives in app/metadata.ts only)
          const beforeMeta = contentToWrite;
          ...
          if (!contentToWrite.includes(footerImport)) {
          ...
          // Create a code change object similar to what the chat API would return
          const codeChangeObject = {
            type: 'code_change',
            explanation: 'Corrección automática de sintaxis para compatibilidad con Next.js App Router',
            changes: [
              {
                file: relativePath,
                replacements: [
                  // Remove next/head imports and Head components
                  {
                    old: "import Head from 'next/head';",
                    new: ''
                  },
                  // Replace fragment wrappers with divs
                  {
                    old: '<>\n',
                    new: '<div className="container mx-auto py-8">\n'
                  },
                  {
                    old: '\n</>',
                    new: '\n</div>'
                  },
                  // Remove any Head components
                  {
          ...
          if (projectId) {
            await applyCodeChanges(codeChangeObject, basePath, projectId);

            // Read the updated file content
            contentToWrite = await fs.readFile(fullPath, 'utf8');
            console.log(`✅ Fixed syntax for: ${relativePath}`);
          } else {
            console.log(`⚠️ Skipping code correction for: ${relativePath} (no projectId provided)`);
          }
        } catch (error) {
          console.warn(`⚠️ Code correction failed for ${relativePath}:`, error);
...
- ⚠️ MANDATORY IMPORT: In app/page.tsx, you MUST include this import at the top: import Footer from '@/components/layout/footer';
- ⚠️ MANDATORY USAGE: You MUST use the Footer component in the JSX. Include <Footer /> at the end of the component's return statement, before closing the main container.
- ⚠️ CRITICAL: DO NOT create inline <footer> tags. ALWAYS use the imported Footer component.
- The Footer component will automatically handle the copyright text: "© ${new Date().getFullYear()} ${appName}. Todos los derechos reservados. Aplicación creada con www.zeus-ia.com"
- The Footer component MUST be included in the JSX structure of the page component.
...
- Modern, consistent, and meaningful iconography
...
5. NAVIGATION AND PAGES RULES (CRITICAL):\n - ONLY create navigation links (in Navbar, Footer, buttons, links, etc.) to pages that ACTUALLY EXIST in the generated file structure.\n - If you include links to pages like /about, /contact, /services, /features, etc., you MUST include the corresponding page files (e.g., app/about/page.tsx, app/contact/page.tsx) in the JSON output with COMPLETE content.\n - NEVER create navigation links, buttons, or <Link> components pointing to pages that don't exist in the file list.\n - If the app only has app/page.tsx, the navigation should ONLY link to "/" or "#" — do NOT invent additional page routes.\n - Every page referenced in navigation MUST have a real file with substantial content, not empty or placeholder content.\n - When creating multi-page apps, ALWAYS include ALL pages in the file structure before adding navigation to them.\n\n6. OUTPUT JSON STRUCTURE:\n - The root key MUST be the name of the app (e.g. "my-awesome-app").\n - Within the root key, there MUST be a "files" key.\n - The "files" key MUST contain an object where the keys are the file paths and the values are the full file content.\n       - Ejemplo:\n         {\n           "my-awesome-app": {\n             "files": {\n               "package.json": "{\n                 \\"name\\":\\"my-awesome-app\\",...",\n               "app/page.tsx": "import React from \'react\'; ..."\n             }\n           }\n         }\n  `;
  ...
  if (fileListResult.usage && userId) {
    try {
      ...
      console.error('[Usage Recording] Error registrando consumo en Lista de Archivos (Web Page):', usageError);
    }
  }
...
- ⚠️ CRITICAL: DO NOT create inline footer elements. ALWAYS use the imported Footer component. If the file is app/globals.css or src/index.css: - MUST include @tailwind directives (base, components, utilities). MUST contain custom CSS variables. MUST include base styles for the body, HTML, etc.. MUST be at least 50 lines of useful content. If the file is a component:
...
`;
        ...
        if (fileContentResult.usage && userId) {
          try {
            ...
            console.error(`[Usage Recording] Error registrando consumo en archivo ${filePath} (Web Page):`, usageError);
          }
        }
    ...
    // Apply code correction for app/page.tsx during streaming
    if (filePath === 'app/page.tsx') {
      console.log(`🔧 Applying code correction during streaming to: ${filePath}`);
      try {
        ...
        // Create a code change object for streaming mode
        const codeChangeObject = {
          type: 'code_change',
          explanation: 'Corrección automática de sintaxis para compatibilidad con Next.js App Router',
          changes: [
        ...
        // Apply the code changes directly if projectId is available
        if (projectId) {
          await applyCodeChanges(codeChangeObject, targetPath, projectId);

          // Read the updated file content
          fileContent = await fs.readFile(fullPath, 'utf8');
          console.log(`✅ Fixed syntax for: ${filePath}`);
        } else {
          console.log(`⚠️ Skipping code correction for: ${filePath} (no projectId provided)`);
        }
      } catch (error) {
        console.warn(`⚠️ Code correction failed for ${filePath}:`, error);
      }
    }
  ...
  console.log('[generate-app-page-web] 💾 Guardado de ZIP en PocketBase deshabilitado. Archivos disponibles en disco local.');
  ...
  if (packageJsonIndex !== -1) {
    ...
    try {
      ...
      if (parsed.name === 'pocketbase-installer') {
        console.warn('⚠️ package.json raíz tenía contenido pocketbase-installer; reemplazando.');
  ...
  // Evitar duplicados de pocket-base/package.json
...
export async function POST(request: Request) {
  try {
    ...
    const {
      ...
      features = [], // authentication, database, chat - para scripts de package.json y pocket-base
    ...
    if (appType === 'web-app') {
      ...
      await createProjectStructure(targetPath, flattenedProjectFiles, requestBody.projectId);
...
`;
      ...
      if (needsPocketBase(features)) {
        ...
        if (startPocketbaseIndex === -1) {
          ...
          console.log('✅ Added scripts/start-pocketbase.js to generated web page files (non-streaming)');
        } else {
          createdFilesArray[startPocketbaseIndex].content = startPocketbaseContent;
          console.log('✅ Updated scripts/start-pocketbase.js in generated web page files (non-streaming)');
        }
      }
