...
- Para CSS: Usa 'perspective-origin: center bottom' en lugar de sintaxis incorrecta`;
  ...
  // Reglas específicas para manejo de audio y reproductor
  const audioRelated = (features || []).join(' ').toLowerCase().match(/audio|player|reproductor|music|mp3|wav/)
    || filePath.toLowerCase().match(/audio|player|reproductor/)
    || description.toLowerCase().match(/audio|player|reproductor|music|mp3|wav/)
    || isPage || isComponent;

  if (audioRelated) {
    basePrompt += `

**REGLAS CRÍTICAS DE AUDIO (OBLIGATORIAS)**:
- NO cambies el estado de reproducción directamente con setState (por ejemplo, setIsPlaying). En su lugar, usa SIEMPRE las funciones expuestas por el hook de audio (por ejemplo, play() y pause()).
- La UI debe derivarse del estado expuesto por el hook (isPlaying, currentTime, duration, volume, etc.).
- Los handlers del UI deben invocar métodos del hook: onClick -> play()/pause(), onChange -> setVolume()/seek(), etc.
- Si implementas un custom hook (p.ej. useAudioPlayer), encapsula allí el manejo del elemento HTMLAudioElement y expón solo métodos/estado; el componente NO debe manipular el elemento ni usar setState para controlar reproducción.
- Asegúrate de limpiar listeners/timers en useEffect para evitar memory leaks.`;
  }
