...
async function generateFileContentWithStreaming(
  ...
  const effectiveModelConfig = normalizeModelConfig(customModelConfig || await getModelConfig());
  ...
  const isOllamaCloud =
    effectiveModelConfig.provider?.toLowerCase().includes('ollama cloud') ||
    effectiveModelConfig.provider?.toLowerCase().includes('ollama_cloud') ||
    effectiveModelConfig.provider?.toLowerCase().includes('ollama-cloud') ||
    effectiveModelConfig.url.includes('ollama.com') ||
    effectiveModelConfig.url.includes('ollama.cloud');
  ...
  return new ReadableStream({
    async start(controller) {
      try {
        ...
        let requestBody;
        const isGoogleApi = effectiveModelConfig.url.includes('generativelanguage.googleapis.com');
        ...
        const apiKey = effectiveModelConfig.apiKey;
        let apiUrl = effectiveModelConfig.url;
